...
CREATE TABLE `cms_channel`  (
  ...
  `pid` varchar(50)  NULL DEFAULT NULL COMMENT '父类ID',
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '栏目表' ;
...
CREATE TABLE `cms_channel_content`  (
  ...
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '栏目内容中间表' ;
...
CREATE TABLE `cms_comment`  (
  `id` varchar(50)  NOT NULL,
  `userId` varchar(50)  NULL DEFAULT NULL COMMENT '评论用户ID',
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = 'CMS评论表' ;
...
CREATE TABLE `cms_content`  (
  ...
  `mintitle` varchar(200)  NULL DEFAULT NULL COMMENT '小标题',
  ...
  `sourceurl` varchar(1000)  NULL DEFAULT NULL COMMENT '来源地址',
  `status` int(11) NULL DEFAULT NULL COMMENT '状态  0未审核  1审核通过',
  `active` int(11) NULL DEFAULT NULL COMMENT '是否可用',
  `commentPerm` int(11) NULL DEFAULT NULL COMMENT '评论开关  0不允许评论  1 允许评论',
...
CREATE TABLE `cms_friend_site`  (
  ...
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '友情链接' ;
...
CREATE TABLE `cms_link`  (
  ...
  `nodeftlfile` varchar(1000)  NULL DEFAULT NULL COMMENT '子内容使用的ftl模板文件',
  `statichtml` int(11) NOT NULL DEFAULT 0 COMMENT '是否静态化 0否,1是',
  `sortno` int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  `jumpType` int(11) NULL DEFAULT NULL COMMENT '跳转方式',
  `loginuser` int(11) NULL DEFAULT NULL COMMENT '是否需要登录访问  0否 1是',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '业务链接表' ;
...
CREATE TABLE `cms_property`  (
  ...
  `createDate` datetime(0) NOT NULL COMMENT '创建时间',
  `defaultValue` varchar(100)  NULL DEFAULT NULL COMMENT '默认值',
  `pvalue` varchar(500)  NULL DEFAULT NULL,
  `style` varchar(500)  NULL DEFAULT NULL COMMENT '样式',
  `leafUse` int(11) NOT NULL DEFAULT 0 COMMENT '自是否可用,0不可用,1可以用',
  `sortno` int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '自定义属性表' ;
...
CREATE TABLE `cms_site`  (
  ...
  `name` varchar(500)  NULL DEFAULT NULL COMMENT '名称',
  `title` varchar(500)  NULL DEFAULT NULL,
  `domainurl` varchar(2000)  NULL DEFAULT NULL COMMENT '网站域名',
  `logo` varchar(2000)  NOT NULL COMMENT '网站logo',
  ...
  `themeId` varchar(50)  NULL DEFAULT NULL COMMENT '主题Id',
  `lookcount` int(11) NULL DEFAULT NULL COMMENT '打开次数',
  `siteType` int(11) NOT NULL DEFAULT 0 COMMENT '11微信,12企业号,13pc,14wap,15投票',
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  `springbeanid` varchar(100)  NULL DEFAULT NULL COMMENT 'springbeanid',
  `orgId` varchar(50)  NULL DEFAULT NULL COMMENT '部门id',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `orgId`(`orgId`) USING BTREE,
  CONSTRAINT `orgId` FOREIGN KEY (`orgId`) REFERENCES `t_org` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '站点表' ;

-- ----------------------------
-- Records of cms_site
-- ----------------------------
INSERT INTO `cms_site` VALUES ('s_PT', NULL, NULL, 'u_10001', '测试站点', '测试站点', NULL, '', '', '', '', '', NULL, NULL, NULL, NULL, 0, 1, NULL, 'o_10003');
...
CREATE TABLE `cms_template`  (
  ...
  `description` varchar(2000)  NULL DEFAULT NULL COMMENT '备注',
  `ftlfile` varchar(1000)  NULL DEFAULT NULL COMMENT '渲染使用的模板路径',
  `imgfile` varchar(1000)  NULL DEFAULT NULL COMMENT '缩略图路径路径',
  `modelType` int(11) NOT NULL DEFAULT 0 COMMENT '0site,1channel,2content,3投票(以后可能扩展更多系统功能,例如 注册 登录 订单 购物车)',
  `usecount` int(11) NULL DEFAULT NULL COMMENT '使用次数',
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `cms_theme`  (
  ...
  `active` int(11) NOT NULL DEFAULT 1 COMMENT '状态 0不可用,1可用',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '主题表' ;
...
CREATE TABLE `cms_theme_template`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4  COMMENT = '主题和模板中间表' ;
