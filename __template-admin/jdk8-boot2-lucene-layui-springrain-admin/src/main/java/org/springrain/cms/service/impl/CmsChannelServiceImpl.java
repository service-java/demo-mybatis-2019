...
public class CmsChannelServiceImpl extends BaseSpringrainServiceImpl implements ICmsChannelService {
	...
	public String saveChannel(CmsChannel cmsChannel) throws Exception {
		...
		if (StringUtils.isEmpty(ftlFile)) {
			...
			if (StringUtils.isBlank(pid)) {
				pid = siteId;
				// 如果父类为空，到站点的
				ftlFile = cmsLinkService
						.findLinkBySiteBusinessIdModelType(siteId, pid, Enumerations.CmsLinkModeType.站点.getType())
						.getNodeftlfile();
			} else {
				// 如果不为空取父类的，默认只给"站长后台列表"的 nodeftlfile，当然其它的保存的时候设置为空了
				ftlFile = cmsLinkService
						.findLinkBySiteBusinessIdModelType(siteId, pid, Enumerations.CmsLinkModeType.站长后台列表.getType())
						.getNodeftlfile();
			}
		}
	...
	public Integer updateChannel(CmsChannel cmsChannel) throws Exception {
		...
		// 修改cmsLink
		CmsLink link = cmsLinkService.findLinkBySiteBusinessIdModelType(siteId, id,
				Enumerations.CmsLinkModeType.站长后台列表.getType());// 只查列表即可
		...
		if (link != null && !link.getNodeftlfile().equals(cmsChannel.getNodeftlPath())) {
			...
			Finder finder = Finder.getUpdateFinder(CmsLink.class,
					" ftlfile=:ftlfile where siteId=:siteId and ftlfile=:oldftlfile and businessId like :bid");
			finder.setParam("ftlfile", cmsChannel.getNodeftlPath()).setParam("siteId", siteId)
					.setParam("oldftlfile", link.getNodeftlfile()).setParam("bid", "c_%");
			super.update(finder);
			// 临时开关处理 开始 只保留 40的nodefltlfile 此处方法为处理老数据，部署完后可注释掉
			finder = Finder.getUpdateFinder(CmsLink.class,
					" nodeftlfile=null where siteId=:siteId and businessId=:businessId and modelType <> 40 ");
		...
		link = cmsLinkService.findLinkBySiteBusinessIdModelType(siteId, id,
				Enumerations.CmsLinkModeType.站长后台更新.getType());
		...
		link = cmsLinkService.findLinkBySiteBusinessIdModelType(siteId, id,
				Enumerations.CmsLinkModeType.站长后台查看.getType());
		...
		// 看前台的croller
		link = cmsLinkService.findLinkBySiteBusinessIdModelType(siteId, id,
				Enumerations.CmsLinkModeType.前台栏目.getType());
		...
		Finder f_old_c = Finder.getSelectFinder(CmsChannel.class, "comcode").append(" WHERE id=:id ").setParam("id",
				id);
		...
		// 级联更新
		Finder f_s_list = Finder.getSelectFinder(CmsChannel.class, "id,comcode")
	...
	public List<CmsChannel> findTreeByPid(String pid, String siteId) throws Exception {
		...
		List<CmsChannel> channelList = null;// getByCache(siteId, "cmsChannelService_findTreeByPid_"+pid+"_"+siteId,
											// List.class);
		if (CollectionUtils.isNotEmpty(channelList)) {
			return channelList;
		}

		Finder finder = Finder.getSelectFinder(CmsChannel.class).append("  WHERE active=:active and siteId=:siteId ")
				.setParam("siteId", siteId).setParam("active", 1);
	...
	public String findChannelNewComcode(String channelId, String pid, String siteId) throws Exception {
		...
		String comcode = null;
		Finder f_p_c = Finder.getSelectFinder(CmsChannel.class, "comcode").append(" WHERE id=:id and siteId=:siteId ")
				.setParam("siteId", siteId).setParam("id", pid);
	...
	@Override
	public List<CmsChannel> findListByIdsForTag(List<String> asList, String siteId, Map params) throws Exception {
		if (StringUtils.isEmpty(siteId) || asList == null || asList.size() <= 0) {
			return null;
		}
		Finder finder = Finder.getSelectFinder(CmsChannel.class);
		finder.append(" where id in (:ids) and siteId = :siteId ").setParam("ids", asList).setParam("siteId", siteId);

		// 常用参数（表与实体类直接对应的字段）
		/*
		 * CmsChannel queryBean = new CmsChannel(); if(params!=null){
	...
	@Override
	public List<CmsChannel> findListForTag(Map params, Environment env, String siteId) throws Exception {
	...
	public CmsChannel findCmsChannelById(String id, String siteId) throws Exception {
		Finder finder = Finder.getSelectFinder(CmsChannel.class);
		finder.append(" where id=:id and siteId=:siteId ").setParam("id", id).setParam("siteId", siteId);
