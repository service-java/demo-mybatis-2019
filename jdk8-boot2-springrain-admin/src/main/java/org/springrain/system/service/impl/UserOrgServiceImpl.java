...
public class UserOrgServiceImpl extends BaseSpringrainServiceImpl implements IUserOrgService {
	...
	public List<User> findUserByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT u.* FROM ").append(Finder.getTableName(User.class)).append("  u,")
				...
				.append(" re where re.userId=u.id and re.orgId=:orgId order by u.id asc ");
	...
	public List<String> findUserIdsByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT re.userId FROM ").append(Finder.getTableName(UserOrg.class))
				.append(" re where  re.orgId=:orgId order by re.userId asc ");
		finder.setParam("orgId", orgId);
	...
	public List<User> findAllUserByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT u.* FROM ").append(Finder.getTableName(User.class)).append("  u,")
				.append(Finder.getTableName(UserOrg.class)).append(" re,").append(Finder.getTableName(Org.class))
				.append(" org WHERE org.id=re.orgId and u.id=re.userId and org.comcode like :comcode  order by u.id asc ");
	...
	public List<String> findAllUserIdsByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT re.userId FROM ").append(Finder.getTableName(UserOrg.class)).append(" re,")
				...
				.append(" org WHERE org.id=re.orgId and org.comcode like :comcode  order by re.userId asc ");
		finder.setParam("comcode", comcode + "%");
		return super.queryForList(finder, String.class);
	}
	...
	public List<Org> findOrgByUserId(String userId) throws Exception {
		...
		Finder finder = new Finder("SELECT org.* FROM  ").append(Finder.getTableName(UserOrg.class)).append(" re ,")
				.append(Finder.getTableName(Org.class))
				.append(" org  WHERE re.userId=:userId and org.id=re.orgId  order by org.id asc   ");
	...
	public List<UserOrg> findManagerOrgByUserId(String userId) throws Exception {
		...
		Finder finder = new Finder("SELECT re.*,org.name  orgName FROM  ").append(Finder.getTableName(UserOrg.class))
	...
	public List<String> findOrgIdsByUserId(String userId) throws Exception {
		...
		Finder finder = new Finder("SELECT re.orgId FROM  ").append(Finder.getTableName(UserOrg.class)).append(" re ")
				.append("   WHERE re.userId=:userId    order by re.orgId asc   ");
		finder.setParam("userId", userId);
		return super.queryForList(finder, String.class);
	}
	...
	public List<User> findManagerUserByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT u.* FROM ").append(Finder.getTableName(User.class)).append(" u,")
				.append(Finder.getTableName(Org.class)).append(" org,").append(Finder.getTableName(UserRole.class))
				.append(" re  WHERE re.roleId=org.managerRoleId and org.id=:orgId and u.id=re.userId order by re.userId asc   ");
		finder.setParam("orgId", orgId);
		return super.queryForList(finder, User.class);
	}
	...
	public List<String> findManagerUserIdsByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT re.userId FROM ").append(Finder.getTableName(Org.class)).append(" org,")
				.append(Finder.getTableName(UserRole.class))
				.append(" re  WHERE re.roleId=org.managerRoleId and org.id=:orgId order by re.userId asc   ");
		finder.setParam("orgId", orgId);
		return super.queryForList(finder, String.class);
	}
	...
	public Integer findAllUserCountByOrgId(String orgId) throws Exception {
		...
		Finder finder = new Finder("SELECT count(re.userId) FROM ").append(Finder.getTableName(UserOrg.class))
				.append(" re,").append(Finder.getTableName(Org.class))
				.append(" org WHERE org.id=re.orgId and org.comcode like :comcode");
		finder.setParam("comcode", comcode + "%");
...
//		userIdsSQLByManagerUserId=userIdsSQLByManagerUserId+" order by  _system_temp_user_org.userId asc ";
	...
	public List<User> findUserByManagerUserId(String managerUserId, Page page) throws Exception {
		...
		finder.append(Finder.getTableName(User.class)).append(" u,").append(Finder.getTableName(UserOrg.class))
				.append(" re,").append(Finder.getTableName(Org.class)).append(" _system_temp_org ");
...
//		hasLeafBuffer.append(" order by _system_temp_org.id  asc "); 
	...
	public Finder findUserIdsSQLByManagerUserId(String managerUserId) throws Exception {
		...
		sb.append(Finder.getTableName(UserOrg.class)).append(" _system_temp_user_org,")
	...
	private Finder wrapWheresSQLByManagerUserId(String managerUserId) throws Exception {
		...
		Finder f1 = new Finder("SELECT * FROM  ").append(Finder.getTableName(UserOrg.class))
				.append(" WHERE  userId=:userId  and managerType in (" + UserOrgType.主管.getType() + ","
						+ UserOrgType.虚拟主管.getType() + "," + UserOrgType.代理主管.getType() + ") ");
