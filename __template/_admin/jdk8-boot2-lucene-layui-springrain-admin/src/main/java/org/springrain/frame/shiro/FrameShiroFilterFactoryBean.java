...
public class FrameShiroFilterFactoryBean implements FactoryBean, BeanPostProcessor {
	 ...
	 * that subclass {@link AccessControlFilter} or {@code null} if no value should
	 * be assigned globally. The default value is {@code null}.
	 *
	 * @return the application's login URL to be assigned to all acquired Filters
	 *         that subclass {@link AccessControlFilter} or {@code null} if no value
	 *         should be assigned globally.
	 * @see #setLoginUrl
	 */
	 ...
	 * subclass {@link AccessControlFilter}. This is a convenience mechanism: for
	 * all configured {@link #setFilters filters}, as well for any default ones
	 * ({@code authc}, {@code user}, etc), this value will be passed on to each
	 * Filter via the {@link AccessControlFilter#setLoginUrl(String)}
	 * method<b>*</b>. This eliminates the need to configure the 'loginUrl' property
	 * manually on each filter instance, and instead that can be configured once via
	 * this attribute.
	 * <p/>
	 ...
	 * acquired Filters that subclass {@link AuthenticationFilter} or {@code null}
	 * if no value should be assigned globally. The default value is {@code null}.
	 *
	 ...
	 * Filters that subclass {@link AuthenticationFilter}. This is a convenience
	 * mechanism: for all configured {@link #setFilters filters}, as well for any
	 * default ones ({@code authc}, {@code user}, etc), this value will be passed on
	 * to each Filter via the {@link AuthenticationFilter#setSuccessUrl(String)}
	 * method<b>*</b>. This eliminates the need to configure the 'successUrl'
	 * property manually on each filter instance, and instead that can be configured
	 * once via this attribute.
	 * <p/>
	 * <b>*</b>If a filter already has already been explicitly configured with a
	 * value, it will <em>not</em> receive this value. Individual filter
	 * configuration overrides this global convenience property.
	 ...
	 * mechanism: for all configured {@link #setFilters filters}, as well for any
	 * default ones ({@code roles}, {@code perms}, etc), this value will be passed
	 * on to each Filter via the
	 * {@link AuthorizationFilter#setUnauthorizedUrl(String)} method<b>*</b>. This
	 * eliminates the need to configure the 'unauthorizedUrl' property manually on
	 ...
	 * defining filter chain definitions. All filter chain definitions will
	 ...
	 * implementation will discover all beans in the web application context that
	 * implement the {@link Filter} interface and automatically add them to this
	 * filter map under their bean name.
	 * <p/>
	 ...
	 * creating filter chains intercepted by the Shiro Filter. Each map entry should
	 * conform to the format defined by the
	 * {@link FilterChainManager#createChain(String, String)} JavaDoc, where the map
	 * key is the chain name (e.g. URL path expression) and the map value is the
	 * comma-delimited string chain definition.
	 *
	 * @return he chainName-to-chainDefinition map of chain definitions to use for
	 *         creating filter chains intercepted by the Shiro Filter.
	 */
	 ...
	 *                                 definitions to use for creating filter chains
	 *                                 intercepted by the Shiro Filter.
	 */
	 ...
	 * {@link #setFilterChainDefinitionMap(java.util.Map) filterChainDefinitionMap}
	 * property by accepting a {@link java.util.Properties Properties}-compatible
	 * string (multi-line key/value pairs). Each key/value pair must conform to the
	 * format defined by the {@link FilterChainManager#createChain(String,String)}
	 * JavaDoc - each property key is an ant URL path expression and the value is
	 * the comma-delimited chain definition.
	 *
	 * @param definitions a {@link java.util.Properties Properties}-compatible
	 *                    string (multi-line key/value pairs) where each key/value
	...
	public void setFilterChainDefinitions(String definitions) {
		...
		if (CollectionUtils.isEmpty(section)) {
			// no urls section. Since this _is_ a urls chain definition property, just
			// assume the
	...
	/**
	 * Lazily creates and returns a {@link AbstractShiroFilter} concrete instance
	 * via the {@link #createInstance} method.
	 *
	 * @return the application's Shiro Filter instance used to filter incoming web
	 *         requests.
	 ...
	 * <li>{@link #createFilterChainManager() Creates} a {@link FilterChainManager}
	 * instance that reflects the configured {@link #setFilters(java.util.Map)
	 * filters} and {@link #setFilterChainDefinitionMap(java.util.Map) filter chain
	 * definitions}</li>
	 * <li>Wraps the FilterChainManager with a suitable
	 * {@link org.apache.shiro.web.filter.mgt.FilterChainResolver
	 * FilterChainResolver} since the Shiro Filter implementations do not know of
	 * {@code FilterChainManager}s</li>
	 ...
	 * </ol>
	 *
	 * @return a new Shiro Filter reflecting any configured filters and filter chain
	 *         definitions.
	 * @throws Exception if there is a problem creating the AbstractShiroFilter
	 *                   instance.
	 */
	 ...
	 * {@link #setFilters(java.util.Map) filters map} that will be referenced later
	 * during filter chain construction.
	 */
	@Override
	...
	/**
	 * Ordinarily the {@code AbstractShiroFilter} must be subclassed to additionally
	 * perform configuration and initialization behavior. Because this
	 * {@code FactoryBean} implementation manually builds the
	...
	private static final class SpringShiroFilter extends AbstractShiroFilter {
		 ...
		 * required by the Servlet Specification when using Shiro-based sessions (and
		 * not Servlet Container HTTP-based sessions).
		 *
		 * @param orig    the original {@code HttpServletResponse} instance provided by
		 *                the Servlet Container.
		 * @param request the {@code ShiroHttpServletRequest} instance wrapping the
		 *                original request.
