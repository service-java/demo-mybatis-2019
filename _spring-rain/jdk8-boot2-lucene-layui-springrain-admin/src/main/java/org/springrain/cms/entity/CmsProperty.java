...
public class CmsProperty extends BaseEntity {
	...
	// alias
	/*
	 * public static final String TABLE_ALIAS = "自定义属性表"; public static final String
	 * ALIAS_ID = "siteId_code,保证一个站点下code唯一"; public static final String
	 * ALIAS_SITEID = "siteId"; public static final String ALIAS_NAME = "name";
	 * public static final String ALIAS_CODE = "系统级别的编码,一个站点不可重复"; public static
	 * final String ALIAS_INPUTTYPE =
	 * "0text,1date,2datatime,3int,4float,5select,6file,7img,8imgs"; public static
	 * final String ALIAS_BUSINESSID = "业务Id"; public static final String
	 * ALIAS_MODELTYPE =
	 * "0site,1channel,2content,3投票(以后可能扩展更多系统功能,例如 注册 登录 订单 购物车)"; public static
	 * final String ALIAS_CREATEPERSON = "创建人"; public static final String
	 * ALIAS_CREATEDATE = "创建时间"; public static final String ALIAS_DEFAULTVALUE =
	 * "默认值"; public static final String ALIAS_STYLE = "样式"; public static final
	 * String ALIAS_SORTNO = "排序"; public static final String ALIAS_ACTIVE =
	 * "状态 0不可用,1可用";
	 */
	// date formats
	...
	public String toString() {
		return new StringBuilder().append("siteId_code,保证一个站点下code唯一[").append(getId()).append("],").append("siteId[")
				.append(getSiteId()).append("],").append("name[").append(getName()).append("],")
				.append("系统级别的编码,一个站点不可重复[").append(getCode()).append("],")
				.append("0text,1date,2datatime,3int,4float,5select,6file,7img,8imgs[").append(getInputType())
				.append("],").append("业务Id[").append(getBusinessId()).append("],")
				.append("0site,1channel,2content,3投票(以后可能扩展更多系统功能,例如 注册 登录 订单 购物车)[").append(getModelType())
				.append("],").append("创建人[").append(getCreatePerson()).append("],").append("创建时间[")
				.append(getCreateDate()).append("],").append("默认值[").append(getDefaultValue()).append("],")
				.append("样式[").append(getStyle()).append("],").append("排序[").append(getSortno()).append("],")
				.append("状态 0不可用,1可用[").append(getActive()).append("],").toString();
	}
