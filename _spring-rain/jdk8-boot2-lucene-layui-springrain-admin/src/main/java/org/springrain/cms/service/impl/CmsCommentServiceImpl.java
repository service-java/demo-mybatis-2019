...
public class CmsCommentServiceImpl extends BaseSpringrainServiceImpl implements ICmsCommentService {
	...
	public Integer findCommentsNumByBusinessId(String siteId, String businessId) throws Exception {
		...
		Finder finder = new Finder("SELECT COUNT(*) FROM ").append(Finder.getTableName(CmsComment.class))
	...
	@Override
	public String saveWechatFile(String serverIds, String siteType, String siteId, String businessId)
		...
		// IWxMpMaterialService wxMpMaterialService = new
		// WxMpMaterialServiceImpl(wxMpService);
		...
		for (String serverId : serverIdArr) {
			...
			String formalPath = GlobalStatic.rootDir + "/upload/" + siteType + "/" + siteId + "/" + businessId + "/"
					+ downLoadFile.getName();
			File formalFile = new File(formalPath);
			FileUtils.copyFile(downLoadFile, formalFile);
			filePath.append("/upload/").append(siteType).append("/").append(siteId).append("/").append(businessId)
					.append("/").append(SecUtils.getUUID()).append(downLoadFile.getName()).append(",");
		}
	...
	@Override
	public List<CmsComment> findListByOpenId(String openId, String siteId, String businessId, Integer type)
			throws Exception {
		...
		finder.append(" where siteId=:siteId and businessId=:businessId and userId=:openId ").setParam("siteId", siteId)
				.setParam("businessId", businessId).setParam("openId", openId);
		if (type != null) {
